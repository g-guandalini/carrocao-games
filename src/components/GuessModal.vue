  ...
  <div v-if="show" class="modal-overlay">
    <div class="modal-content">
      ...
      <div class="modal-actions">
        <!-- Botão "Confirmar Palpite" agora sempre ativo -->
        <button @click="submit">Confirmar Palpite</button>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const submit = () => {
      if (currentGuess.value.trim()) { // Esta verificação impede palpites vazios ou com apenas espaços
...
.modal-actions {
  display: flex; /* Faz os itens internos (botões) se organizarem em linha */
  justify-content: center; /* Centraliza os botões horizontalmente */
  gap: 20px; /* Adiciona um espaço de 20px entre os botões */
  /* flex-wrap: wrap; */
}

.modal-actions button {
  padding: 12px 25px;
  /* margin: 0 10px; */ /* Removido para usar o 'gap' do flexbox */
  border: none;
  border-radius: 7px;
  cursor: pointer;
  font-size: 1.1em;
...
/* Remover este bloco de CSS pois o botão não será mais desabilitado */
/*
